...
int main() {
    ...
    for(int i = 0; i < NUM_CHILDREN; i++){
        fds[i].fd = pipe_fds[2*i];  // only need to monitor read ends of the pipes
        fds[i].events = POLLIN | POLLHUP; // Check for read
    }
    int result;
    ...
    while(remaining > 0){
        if (poll(fds, NUM_CHILDREN, -1) > 0){
            for(int i = 0; i < NUM_CHILDREN; i++){
                if(fds[i].revents & POLLIN){    // pipe is ready for a read
